...
static void clipPolygons(HDC dc, const Vector<PathPolygon>& polygons, RECT boundingRect, const AffineTransform* transformation)
{	
	int pointCount = 0;
	for (Vector<PathPolygon>::const_iterator i = polygons.begin(); i != polygons.end(); ++i) {
		pointCount += i->size();
	}
    //nado sozdat map chtob ne plodit s odinakovamy znacheniyami elementy massiva, no eto mozhet byt dorogo, nado glyanut na slozhnost O
	DWORD dwSize = pointCount*sizeof(POINT);
	POINT* winPoints = (POINT*)malloc(dwSize);
    pointCount = 0;
	for (Vector<PathPolygon>::const_iterator i = polygons.begin(); i != polygons.end(); ++i) {
		int npoints = i->size();
		...
		for (int j = 0; j < npoints; ++j) {
			FloatPoint trPoint = transformation->mapPoint(i->at(j));
			winPoints[pointCount].x = stableRound(trPoint.x());
			winPoints[pointCount].y = stableRound(trPoint.y());
			++pointCount;
		}
	}
    ...
    for (int i = 0;  i < pointCount; ++i){            
        for (int j=0;  j < pointCount - 1;  ++j){     
			if (winPoints[j].y > winPoints[j+1].y){     
                pointTemp = winPoints[j];       
				winPoints[j] = winPoints[j+1]; 
                winPoints[j+1] = pointTemp; 
            }
        }
    }
	...
	for (int i = 0;  i < pointCount; ++i){            
        for (int j=0;  j < pointCount - 1;  ++j){     
			if (winPoints[j].y == winPoints[j+1].y && winPoints[j].x > winPoints[j+1].x){     
                pointTemp = winPoints[j];       
				winPoints[j] = winPoints[j+1]; 
                winPoints[j+1] = pointTemp; 
            }
	...
	dwSize = (winPoints[pointCount-1].y - winPoints[0].y + 1) * sizeof(RECT);
	RECT* rects = (RECT*)malloc(dwSize);
	...
	for (int j=0;  j < pointCount - 1;  ++j){     
		if (winPoints[j].y == winPoints[j+1].y){  
			if(!isNewLine) 
			{
				x1 = winPoints[j].x;
				y1 = winPoints[j].y;
				if(flag) y2 = winPoints[j-1].y;
				isNewLine = true;;
			}
			else
				x2 = winPoints[j+1].x;

			if(j == pointCount - 2) goto label;
		}                                 
		else if(flag == false && x1 >= winPoints[j+1].x) 
		{
			rects[nUsedRects].left = x1;
			rects[nUsedRects].top = y1;
			rects[nUsedRects].right = x2;
			rects[nUsedRects].bottom = winPoints[j+1].y;
			isNewLine = false;
			++nUsedRects;
		}
		else if(flag == false) 
...
label:
			rects[nUsedRects].left = x1;
			rects[nUsedRects].top = y2;
			rects[nUsedRects].right = x2;
			rects[nUsedRects].bottom = y1;
			isNewLine = false;
			++nUsedRects;
		}	
	}

	dwSize = sizeof(RGNDATAHEADER) + nUsedRects * sizeof(RECT);
	RGNDATA *pRgnData = (RGNDATA *)malloc(dwSize);

	pRgnData->rdh.dwSize=sizeof(RGNDATAHEADER);
	pRgnData->rdh.iType=RDH_RECTANGLES;
	pRgnData->rdh.nCount=nUsedRects;
	pRgnData->rdh.nRgnSize=nUsedRects * sizeof(RECT);
	pRgnData->rdh.rcBound = boundingRect;

	RECT* pRect;
	for (int i = 0; i < nUsedRects; ++i) {
		pRect = &(((RECT *)&pRgnData->Buffer)[i]);
		pRect->bottom = rects[i].bottom;
		pRect->left = rects[i].left;
		pRect->right = rects[i].right;
		pRect->top = rects[i].top;
		//printf("left%d top%d right%d bottom%d\n", pRect->left, pRect->top, pRect->right, pRect->bottom);
	}

	HRGN hRgn = ExtCreateRegion(NULL, sizeof(RGNDATAHEADER) + (nUsedRects * sizeof(RECT)), pRgnData);
...
void PlatformPath::clipPath(HDC dc, RECT rcBound, const AffineTransform* transformation) const
{
	clipPolygons(dc, m_subpaths, rcBound, transformation);
}
