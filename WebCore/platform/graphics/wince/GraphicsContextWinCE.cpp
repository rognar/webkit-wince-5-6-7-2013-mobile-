...
{
	if (paintingDisabled() || !m_data->m_dc || path.isNull())
        return;
	...
	RECT rcBound;
	rcBound.left = stableRound(trRect.x());
	rcBound.top = stableRound(trRect.y());
	rcBound.right = stableRound(trRect.maxX());
	rcBound.bottom = stableRound(trRect.maxY());

    path.platformPath()->clipPath(m_data->m_dc, rcBound, &tr);	
}
...
{
	if (paintingDisabled() || !c.isValid())
        return;

	
	Path path;
    path.addRoundedRect(fillRect, topLeft, topRight, bottomLeft, bottomRight);
	...
	if(getShadow(shadowOffset, shadowBlur, shadowColor, shadowColorSpace))
	{
		ScopeDCProvider dcProvider(m_data);
		if (!m_data->m_dc)
			return;
