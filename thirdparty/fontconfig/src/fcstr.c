...
{
#ifdef WIN32
    /*FcChar16 full[FC_MAX_FILE_LEN + 2];
	FcChar8* full2;
	wchar_t* ws = wce_mbtowc(s);
    int size = GetFullPathNameW (ws, sizeof (full) -2,
				full, NULL);

    if (size == 0)
	perror ("GetFullPathName");

	full2 = wce_wctomb(full);

    FcConvertDosPath (full2);
    return FcStrCanonAbsoluteFilename (full2);*/
	return s;
#else
    if (s[0] == '/')
