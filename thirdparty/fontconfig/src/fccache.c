...
FcStat (const char *file, struct stat *statb)
{
	...
	//wchar_t full_path_namew[MAX_PATH];
    char *basename;
	wchar_t* wfile = (wchar_t *)malloc( sizeof( wchar_t )*MAX_PATH);
    DWORD rc;
	wchar_t* wfile2 = (wchar_t *)malloc( sizeof( wchar_t )*MAX_PATH);

	/*mbstowcs(wfile, "\\Program Files\\ttt\\", MAX_PATH);
	mbstowcs(wfile2, file, MAX_PATH);
	wcscat(wfile, wfile2);*/
	mbstowcs(wfile, file, MAX_PATH);
    
	/*wfile = wce_mbtowc("\\Program Files\\ttt\\");
	wfile2 = wce_mbtowc(file);
	wcscat(wfile, wfile2);*/
	//free(wfile2);

	//GetModuleFileNameW(NULL, full_path_namew, MAX_PATH);
...
FcDirCacheMapFd (int fd, struct stat *fd_stat, struct stat *dir_stat)
{
...
#elif defined(_WIN32)
	{
	    ...
	    cache = NULL;
	    hFileMap = CreateFileMappingW((HANDLE) _get_osfhandle(fd), NULL,
...
FcDirCacheWrite (FcCache *cache, FcConfig *config)
{
	...
	if (/*access ((char *) test_dir, W_OK|X_OK) == 0*/ 1)
	{
	    cache_dir = test_dir;
	    break;
	}
